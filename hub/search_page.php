...
<div class="videos">
    ...
    <div class="videos-content">
        <?php
        // Modify the SQL query to fetch video data
        $video_sql = "SELECT * FROM YOUR_VIDEO_TABLE WHERE ..."; // Add your conditions
        $video_result = $conn->query($video_sql);

        while ($video_row = $video_result->fetch_assoc()) {
            echo "<div class='video-item'>";
            echo "<h3>{$video_row['video_title']}</h3>";
            echo "<iframe width='560' height='315' src='{$video_row['video_link']}' frameborder='0' allowfullscreen></iframe>";
            echo "</div>";
        }
